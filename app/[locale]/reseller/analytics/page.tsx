...
import { TrendingUp, TrendingDown, Users, DollarSign, CheckCircle, Clock, FileDown, BarChart3 } from 'lucide-react';
...
export default function ResellerAnalyticsPage() {
  ...
  const fetchAnalytics = async () => {
    ...
    setError(null);
    try {
      const response = await fetch(`/api/analytics/reseller?range=${dateRange}`);
      if (response.ok) {
  ...
  const exportReport = async () => {
    setExporting(true);
    try {
      const response = await fetch(`/api/commissions/export?range=${dateRange}`);
      if (response.ok) {
    ...
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">
              {t('analytics.average_commission')}
            </CardTitle>
            <BarChart3 className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">
              {formatCurrency(data.metrics.averageCommission)}
            </div>
            <div className="text-xs text-muted-foreground mt-1">
              {t('analytics.conversion_rate')}: {data.metrics.conversionRate.toFixed(1)}%
            </div>
          </CardContent>
        </Card>
      </div>
